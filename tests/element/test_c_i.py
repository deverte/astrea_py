...
def test_f_vs_T_KK():
    ...
    assert len(c_i.f_vs_T_KK()) == 158
    f_vs_T_KK_defined = c_i.f_vs_T_KK()[c_i.STRUCTURE.LS_He_2s2_2p2_3P0.value][c_i.STRUCTURE.LS_He_2s2_2p2_3P1.value]
    f_vs_T_KK_undefined = c_i.f_vs_T_KK()[c_i.STRUCTURE.LS_He_2s2_2p2_3P0.value][c_i.STRUCTURE.LS_He_2s2_2p_4d_3Do3.value]
    assert (f_vs_T_KK_defined[0] == np.array([1000.0, 2500.0, 5000.0, 10000.0, 20000.0, 30000.0, 40000.0, 50000.0, 100000.0, 200000.0, 500000.0])).all()
    assert (f_vs_T_KK_defined[1] == np.array([3.57E-02, 1.70E-01, 3.19E-01, 4.24E-01, 4.73E-01, 4.88E-01, 4.95E-01, 5.00E-01, 5.06E-01, 4.88E-01, 4.00E-01])).all()
...
def test_sigma_vs_nu_K():
    ...
    assert len(c_i.sigma_vs_nu_K()) == 158

    keys = [v.value for v in [
        c_i.STRUCTURE.LS_He_2s2_2p2_3P0,
    ]]
    sigma_vs_nu_K = [c_i.sigma_vs_nu_K()[key] for key in keys]
    assert sigma_vs_nu_K[0].shape == (2, 555)
    assert sigma_vs_nu_K[0][0][0] == 3.289780E+17
    assert sigma_vs_nu_K[0][1][0] == 1.730E-20
